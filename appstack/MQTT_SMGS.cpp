...
void MQTT_SMGS_OnMessage(MQTT_Cfg_t &cfg,MQTT_Message_t & msg)
{
...
协议栈相关
*/
const char *GateWayName="W600Demo";
char GateWaySerialNumber[32]="W600";
...
static bool SMGS_MessagePublish(struct __SMGS_gateway_context_t *ctx,const char * topic,void * payload,size_t payloadlen,uint8_t qos,int retain)
{
    ...
    m.payload=std::string((char *)payload,payloadlen);
    m.qos=qos;
    m.retain=retain;
    return MQTT_Publish_Message(m);
}
...
#define MACADDR_LENGTH 6
        ...
        printf("%s:SerialNumber is %s\r\n",TAG,GateWaySerialNumber);
